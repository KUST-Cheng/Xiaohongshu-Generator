...
const ControlPanel: React.FC<Props> = ({
  ...
  const getProgressText = () => {
      ...
      return progress < 50 ? "正在撰写爆款文案..." : "正在绘制高清封面...";
  };
    ...
    <div className="w-full lg:w-1/3 p-6 flex flex-col gap-6 bg-white border-r border-gray-200 overflow-y-auto z-10 shadow-lg scrollbar-hide h-screen">
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          <div className="relative">
              <input
                ...
                className="w-full pl-4 pr-24 py-4 bg-gray-50 border border-gray-200 rounded-2xl focus:ring-2 focus:ring-red-500 focus:border-transparent outline-none transition-all font-medium"
                onKeyDown={(e) => {
              ...
              <button 
                ...
                className="absolute right-2 top-1/2 -translate-y-1/2 px-3 py-2 bg-red-50 text-[#ff2442] text-xs font-bold rounded-xl hover:bg-red-100 transition-colors flex items-center gap-1 disabled:opacity-50"
              >
            ...
            <div className="flex flex-wrap gap-2 mt-3 animate-fadeIn">
                {suggestions.map((s, i) => (
                    <button
                        ...
                        className="text-xs bg-gradient-to-r from-red-50 to-pink-50 text-[#ff2442] px-3 py-2 rounded-full border border-red-100 hover:border-red-300 hover:shadow-sm transition-all font-bold"
                    >
                        {s}
                    </button>
                ))}
          ...
          <div className="grid grid-cols-2 gap-3">
            {STYLES.map((s) => (
              ...
              >
                ...
                <div className={`font-black text-sm ${style === s.id ? 'text-[#ff2442]' : 'text-gray-900'}`}>{s.name}</div>
        ...
        <div className="p-5 rounded-2xl bg-gray-50 border border-gray-100 shadow-inner">
           <label className="block text-sm font-black text-gray-800 mb-4 flex items-center gap-1.5">
           ...
           <div className="flex bg-gray-200/60 p-1 rounded-xl mb-5">
              <button onClick={() => setCoverMode('auto')} className={`flex-1 py-2 rounded-lg text-xs font-black transition-all ${coverMode === 'auto' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>自动生成</button>
              <button onClick={() => setCoverMode('ref')} className={`flex-1 py-2 rounded-lg text-xs font-black transition-all ${coverMode === 'ref' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>参考图</button>
              <button onClick={() => setCoverMode('template')} className={`flex-1 py-2 rounded-lg text-xs font-black transition-all ${coverMode === 'template' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>爆款模板</button>
           </div>
                   ...
                   >
                       {referenceImage ? (
                           ...
                           <>
                              ...
                              <p className="text-[10px] text-gray-400 mt-1">AI 将提取其审美风格</p>
                           </>
                       )}
               ...
               {coverMode === 'template' && (
                   <div className="space-y-4">
                      <div className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm space-y-3">
                          <label className="text-[10px] font-black text-gray-400 block mb-1 uppercase tracking-widest">模板编辑器</label>
                          <div className="grid grid-cols-2 gap-3">
                          ...
                          <div className="py-1">
                             <label className="text-[10px] font-bold text-gray-400 block mb-2 uppercase tracking-wide">标题颜色</label>
                          ...
                          <textarea placeholder="高亮金句" value={memoData.highlight} onChange={(e) => setMemoData({...memoData, highlight: e.target.value})} className="bg-[#FFE66D]/20 border border-[#FFE66D] rounded-xl p-3 text-sm w-full resize-none h-14 outline-none font-bold" />
        ...
        {error && (
          <div className="p-4 bg-red-50 border border-red-100 rounded-2xl flex flex-col gap-3 animate-fadeIn">
            <div className="flex items-start gap-2.5 text-red-600 text-xs leading-relaxed">
            ...
            {(error.includes("配额") || error.includes("quota")) && (
              <a 
                ...
                target="_blank" 
                rel="noreferrer"
                className="flex items-center justify-center gap-1.5 py-2.5 px-4 bg-red-600 text-white text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-red-700 transition-colors shadow-lg shadow-red-100"
              >
                检查计费与配额 <ExternalLink size={12} />
              </a>
            )}
          </div>
        )}
            ...
            {loading ? (
                <div className="space-y-2 animate-fadeIn">
                     <div className="w-full h-14 bg-gray-100 rounded-2xl flex items-center px-5 relative overflow-hidden border border-gray-200">
                         <div 
                             className="absolute left-0 top-0 bottom-0 bg-red-100 transition-all duration-300"
                ...
                <button
                    onClick={onGenerate}
                    disabled={!topic}
                    className="w-full py-5 bg-gradient-to-br from-[#ff2442] to-[#e61d3a] text-white font-black rounded-2xl shadow-xl shadow-red-200 transform active:scale-95 transition-all flex items-center justify-center gap-3 disabled:opacity-30 disabled:grayscale group"
                >
