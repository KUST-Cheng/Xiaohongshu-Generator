...
const App: React.FC = () => {
  ...
  return (
    <div className="flex flex-col lg:flex-row h-screen bg-white text-gray-800 overflow-hidden font-sans">
      {/* 错误提示横幅 (针对开发者) */}
      {error.includes("DEPLOYMENT_CONFIG_ERROR") && (
        <div className="fixed top-0 left-0 right-0 z-[100] bg-gray-900 text-white p-4 flex items-center justify-center gap-3">
          <Terminal size={20} className="text-red-400" />
          <span className="text-sm font-mono">检测到环境变量 API_KEY 缺失。请在部署平台的后台设置中添加它。</span>
        </div>
      )}
